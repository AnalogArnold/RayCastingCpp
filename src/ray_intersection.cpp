...
IntersectionOutput intersect_plane(const Ray &ray,
    ...
    EiVectorD3d plane_normals = cross_rowwise(edge0, nEdge2); // not normalised!
    ...
    Eigen::Array<double, Eigen::Dynamic, 1> barycentric_u =
            ((t_vec * p_vec).rowwise().sum()).array() * inverse_determinants; // shape (faces, 1)
    ...
    Eigen::Array<double, Eigen::Dynamic, 1> barycentric_v =
            (ray_directions.array() * q_vec).rowwise().sum().matrix().array() * inverse_determinants; // shape (faces, 1)
    ...
    Eigen::Array<double, Eigen::Dynamic, 1> t_values =
            (nEdge2.array() * q_vec).rowwise().sum().array() * inverse_determinants; // shape (faces, 1)
